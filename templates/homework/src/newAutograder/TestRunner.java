        ...
        return new RunNotifier() {
            ...
            public void fireTestFailure(Failure failure) {
                ...
                if (failure.getMessage() != null) {
                    Results.instance.addTestFailure(getName(),
                            failure.toString().replace("\"", "'"));
                } else {
                    Results.instance.addTestFailure(getName(),
                            failure.getTestHeader() + ": Test failed");
                }
    ...
    private void writeEndMessage() {
        synchronized (TestRunner.class) {
            ...
            if (numCompleted == numRunners) {
                ...
                double allPercentagePassed =
                        (double) allTestsPassedCount / ((double) totalTestsExecuted
                                - Configuration.instance.getExtraCreditTests()) * 100.0;
                outputWriter.println(
                        "------------------------------------------------------------------");
                outputWriter.printf("%5d   %8d   %10.1f%%   %-15s\n", totalTestsExecuted,
